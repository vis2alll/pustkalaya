...
#                self.pcs._event.app.editor.focus(self.WINDOW)   
    ...
    #                    self.pcs._event.app.editor.error("page"+str(self.latest_page))
            ...
            self.pcs.menu_lvl="1"
#            self.pcs._event.app.editor.error('IOError:'+str(e)[0:40])
...
##        self.pcs.choice_selection(self.pcs._event, choice)
    ...
    #                self.pcs._event.current_window.buffer.reset()
                    ...
                    self.pcs._event.app.editor.error("Error, server replied with", data.status_code)
        
            except IOError, e:
                self.pcs.menu_lvl="1"
                self.pcs._event.app.editor.error(str(e.message))
...
#                ftp.quit()
                ...
                self.pcs._event.app.editor.error('Download completed '+str(filename))
            ...
            self.pcs._event.app.editor.error('dwld: '+str(e.message)[0:60])    # nfp        
...
#                            all_ids.append(book.getElementsByTagName('id')[0].firstChild.nodeValue)
    ...
    def logout(self):
        ...
        if self.login_status==True:
            ...
            self.userid=""
            self.password=""   
            self.pcs._event.app.editor.error('logged out')
        else:
